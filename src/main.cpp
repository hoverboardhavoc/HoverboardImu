...
void setup()
{
  ...
  {
    SERIAL_PORT.println("whoami does not match. Halting...");
    delay(1000);
  }
  ...
  while ((stat != ICM_20948_Stat_Ok) || (whoami != ICM_20948_WHOAMI))
  {
    ...
    SERIAL_PORT.print("whoami does not match (0x");
    SERIAL_PORT.print(whoami, HEX);
    SERIAL_PORT.print("). Halting...");
    SERIAL_PORT.println();
    delay(1000);
  }
